...
func TestLinkedList_SwapFirstAndLast(t *testing.T) {
	tests := []struct {
		name      string
		items     []int
		result    string
		...
		{"even-length list", []int{1, 2, 3, 4, 5, 6}, "nil<-6<->2<->3<->4<->5<->1->nil", 6, 6, 1},
		{"odd-length list", []int{1, 2, 3, 4, 5, 6, 7}, "nil<-7<->2<->3<->4<->5<->6<->1->nil", 7, 7, 1},
	}
